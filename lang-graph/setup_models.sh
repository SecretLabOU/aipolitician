...
MAIN_CODE_DIR="$(dirname "$(pwd)")"
echo "Looking for main code in: ${MAIN_CODE_DIR}"
...
if [ -f "${MAIN_CODE_DIR}/chat_trump.py" ] && [ -f "${MAIN_CODE_DIR}/chat_biden.py" ]; then
  ...
  echo "Found chat_trump.py and chat_biden.py in ${MAIN_CODE_DIR}"
else
  echo -e "${YELLOW}!${NC} Could not find main code files at ${MAIN_CODE_DIR}"
  echo "Files expected:"
  echo "- ${MAIN_CODE_DIR}/chat_trump.py"
  echo "- ${MAIN_CODE_DIR}/chat_biden.py"
